...
def process(st, img):
    ...
    if mode == "Rotate":
        ...
        # Horizontal：水平翻轉
        choice = st.sidebar.selectbox("翻轉方向", ["水平", "垂直", "同時"])
        code = {"Horizontal":1, "Vertical":0, "Both":-1}[choice]
        return cv2.flip(img, code)

    else:  # Affine shear
        pts1 = np.float32([[0,0], [w-1,0], [0,h-1]])
        shift = st.sidebar.slider("剪切係數", -0.5, 0.5, 0.0)
